...
async function main() {
  ...
  const compiledVanillaAuthenticator = json.parse(
    fs
      .readFileSync(
        ARTIFACTS_PATH+'/contracts/starknet/Authenticators/Vanilla.cairo/Vanilla.json'
      )
      .toString('ascii')
  );
  ...
  const compiledEthSigAuthenticator = json.parse(
    fs
      .readFileSync(
        ARTIFACTS_PATH+'/contracts/starknet/Authenticators/EthSig.cairo/EthSig.json'
      )
      .toString('ascii')
  );
  const compiledVanillaVotingStrategy = json.parse(
    fs
      .readFileSync(
        ARTIFACTS_PATH+'/contracts/starknet/VotingStrategies/Vanilla.cairo/Vanilla.json'
      )
      .toString('ascii')
  );
  const compiledEthBalanceOfVotingStrategy = json.parse(
    fs
      .readFileSync(
        ARTIFACTS_PATH+'/contracts/starknet/VotingStrategies/EthBalanceOf.cairo/EthBalanceOf.json'
      )
      .toString('ascii')
  );
  const compiledVanillaExecutionStrategy = json.parse(
    fs
      .readFileSync(
        ARTIFACTS_PATH+'/contracts/starknet/ExecutionStrategies/Vanilla.cairo/Vanilla.json'
      )
      .toString('ascii')
  );
  const compiledZodiacExecutionStrategy = json.parse(
    fs
      .readFileSync(
        ARTIFACTS_PATH+'/contracts/starknet/ExecutionStrategies/EthRelayer.cairo/EthRelayer.json'
      )
      .toString('ascii')
  );
  const compiledSpaceFactory = json.parse(
    fs
      .readFileSync(ARTIFACTS_PATH+'/contracts/starknet/SpaceFactory.cairo/SpaceFactory.json')
      .toString('ascii')
  );
  const compiledSpace = json.parse(
    fs
      .readFileSync(ARTIFACTS_PATH+'/contracts/starknet/SpaceAccount.cairo/SpaceAccount.json')
      .toString('ascii')
  );
  ...
  const deployTxs = [
    ...
    provider.deployContract({ contract: compiledEthBalanceOfVotingStrategy, constructorCalldata: [fossilFactRegistryAddress, fossilL1HeadersStoreAddress]  }),
